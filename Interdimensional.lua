...
task.spawn(function()while true do task.wait(0.1)if S.immortal and ROLE=="BOT"then local c=pl.Character if c then local h=c:FindFirstChildOfClass("Humanoid")if h then pcall(function()h:SetStateEnabled(Enum.HumanoidStateType.Dead, false) h.Health=0 end)end end end end end)
