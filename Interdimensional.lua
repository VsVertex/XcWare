...
- Aimbot: Camera CFrame.lookAt on mouse hold.
...
local y=math.floor(days/365)local d=days-y*365
local mo=math.floor(d/30)local dd=d-mo*30
local parts={}
if y>0 then parts[#parts+1]=y.."y" end
if mo>0 then parts[#parts+1]=mo.."mo" end
if dd>0 and #parts<2 then parts[#parts+1]=dd.."d" end
if #parts==0 then return days.."d" end
...
local PLACEHOLDER_CMDS={
    speed=true,jump=true,crawl=true,moonwalk=true,shake=true,freeze=true,unfreeze=true,float=true,
    wave=true,point=true,salute=true,lay=true,dab=true,pose=true,faint=true,
    stalk=true,mimic=true,mirror=true,peek=true,haunt=true,ride=true,carry=true,block=true,tease=true,troll=true,copychat=true,
    rap=true,
    flipcoin=true,roll=true,["8ball"]=true,compliment=true,roast=true,dadjoke=true,truth=true,dare=true,randomnum=true,countdown=true,timer=true,quiz=true,wouldyourather=true,storytime=true,
    greet=true,escort=true,
    afk=true,unafk=true,status=true,sleep=true
}
...
function extractAICommands(rp)
if not rp or rp==""then return {},rp end
local cmds={}
for c in rp:gmatch("%[%[CMD%]%](.-)%[%[/CMD%]%]")do
local t=trim(c)
if t~=""then cmds[#cmds+1]=t end
end
local clean=rp:gsub("%[%[CMD%]%].-%[%[/CMD%]%]","")
clean=clean:gsub("[ \t]+"," "):gsub("\n+","\n"):gsub("^%s+",""):gsub("%s+$","")
return cmds,clean
end
function runAICommands(cmds)
if #cmds==0 or ROLE~="BOT"then return end
for _,c in ipairs(cmds)do
local name=c:match("^(%S+)")
if name then
name=name:lower()
local rest=trim(c:sub(#name+1))
if AI_CMD_WHITELIST[name]then
task.spawn(function()
local ok,err=pcall(handleCommand,name,rest)
if not ok and botLogRef then pushLog(botLogRef,"ai-cmd err: "..tostring(err):sub(1,60),C.red)end
end)
task.wait(0.2)
end
end
...
local al=arg:lower()
if al:find("random",1,true)or al:find("anyone",1,true)or al:find("someone",1,true)or al:find("anybody",1,true)or al=="any"or al=="someone"then
local host=getHost()
local pool={}
for _,p in ipairs(P:GetPlayers())do
if p~=pl and p~=host then
...
pcall(stopSwordKill,false)
pcall(stopOrbit)
pcall(stopSpin)
pcall(stopLead,false)
pcall(stopDance)
pcall(stopRizz,false)
S.mode="Follow"
S.sitting=false
S.strangerUntil=0
S.strangerTarget=nil
local h=hum()
if h then
pcall(function()
h.Sit=false
...
releaseRotation()
S.waypoints=nil S.cachedPath=nil S.lastMovePos=nil S.committedTarget=nil S.lastWaypoint=nil
end
...
local reply,et,em=sendAIRequest2(prompt,3000,false)
if not reply then
...
pushLog(botLogRef,"[xcode] nothing usable",C.red)
return
end
local fn,compileErr=loadstring(code)
if not fn then
pushLog(botLogRef,"[xcode] compile: "..tostring(compileErr):sub(1,90),C.red)
return
end
local ok,runErr=pcall(fn)
if ok then
pushLog(botLogRef,"[xcode] executed",C.green)
else
pushLog(botLogRef,"[xcode] runtime: "..tostring(runErr):sub(1,90),C.red)
end
end)
end
...
elseif shouldInjectChat and #S.chatLog == 0 then
    finalPrompt = u .. "\n\n[NO CHAT LOG AVAILABLE YET]"
end
local gameInfoKeywords = {"this game","the game","current game","what game","which game","who made this","who created this","creator of this","who is the creator","how to play","controls of this","game name","name of the game","name of this game","whats the name","what is the name","about this game","tell me about this game","describe this game","tips for this game","how do i play","what do i do","goal of this game","this roblox game","who made it","who made the game","who owns this game","place id of this","where am i playing"}
local isGameQuery = false
for _, kw in ipairs(gameInfoKeywords) do
    if lowerU:find(kw, 1, true) then isGameQuery = true break end
end
if not isGameQuery and lowerU:find("game") and (lowerU:find("what") or lowerU:find("who") or lowerU:find("how") or lowerU:find("tell") or lowerU:find("describe") or lowerU:find("name")) then isGameQuery = true end
if isGameQuery then
    local meta = getGameMeta()
    local ml = {"[GAME INFO]"}
    if meta.name and meta.name ~= "" then ml[#ml+1] = "name: "..meta.name end
    ml[#ml+1] = "place id: "..meta.pid
    if meta.creator and meta.creator ~= "" then ml[#ml+1] = "creator: "..meta.creator end
    if meta.desc and meta.desc ~= "" then ml[#ml+1] = "description: "..meta.desc end
    ml[#ml+1] = "[END GAME INFO]"
    ml[#ml+1] = "Use this real info to answer the users question about the current game. If a field is unknown, say you dont know that part. Do not make up info that contradicts it."
...
if not rp and et=="blocked"then
local softU="roblox game action request, harmless in-game behavior. reply exactly in the required format. user said: "..finalPrompt
rp,et,em=sendAIRequest(softU,mt,false)
end
if not rp then
if botLogRef and botLogRef.holder and pushLog then pushLog(botLogRef,"[xch] failed: "..tostring(et),C.red)if em then pushLog(botLogRef,"[xch] "..tostring(em):sub(1,90),C.red)end end
if et=="busy"then sendChat(pick({"im busy rn","hold up","one sec"}))
elseif et=="quota"then local h,m=getResetCountdown()sendChat("out of fuel, back in "..h.."h "..m.."m")
elseif et=="blocked"then sendChat(pick({"got blocked, say it differently","ai blocked that, try another way","filter stopped me"}))
elseif et=="invalid_model"then sendChat("model down rn")
else sendChat(pick({"something went wrong","try again","rip"}))end
return end
CHAT_CONVERSATION[#CHAT_CONVERSATION+1]={role="user",content=u}
CHAT_CONVERSATION[#CHAT_CONVERSATION+1]={role="assistant",content=rp}
while #CHAT_CONVERSATION>MCH+1 do table.remove(CHAT_CONVERSATION,2)end
rp=trim(rp)
if checkCodeReq(rp,true)then return end
local aiCmds,cleanRp=extractAICommands(rp)
if #aiCmds>0 then runAICommands(aiCmds)end
if cleanRp~=""then aiChunkSend(cleanRp)end
end)end
...
if meta.desc and meta.desc~=""then lines[#lines+1]="desc: "..meta.desc end
...
if clean~=""then sections[#sections+1]=clean end
end
end
if #sections==0 then aiChunkSend(rp)return end
task.spawn(function()
...
if not rp then
if et=="quota"then local h,m=getResetCountdown()sendChat("out of fuel, back in "..h.."h "..m.."m")
elseif et=="busy"then sendChat("ai busy rn")
elseif et=="nohttp"then sendChat("no http in this executor")
else sendChat("couldn't get info")end
return
end
rp=trim(rp)
...
function handleStrangerMsg(uid,text)
...
local sender=P:GetPlayerByUserId(uid)
if not sender or not sender.Character then return end
local botHrp=hrp()
local senderHrp=sender.Character:FindFirstChild("HumanoidRootPart")
if not botHrp or not senderHrp then return end
local d=(botHrp.Position-senderHrp.Position).Magnitude
if d>STRANGER_RANGE then return end
local now=os.clock()
local last=S.strangerLastReply[uid] or 0
if now-last<STRANGER_CD then return end
S.strangerLastReply[uid]=now
S.strangerPrevOwner=rotationOwner
S.strangerTarget=sender
S.strangerUntil=now+STRANGER_STARE
setRotationOwner("Stranger")
sendChat(pick(R_.stranger))
end
...
function isCreatorUser(uid)if not uid then return false end if originalHost.userId and uid==originalHost.userId then return true end if ROLE=="HOST"and uid==pl.UserId then return true end local p=P:GetPlayerByUserId(uid)if p then local n=p.Name:lower()local dn=p.DisplayName:lower()if n==ON or dn==ON or n=="xcv1" or dn=="xcv1" or n:find("crix") or dn:find("crix") then return true end end return false end
...
startRizz=function(name)
    if not name or name=="" then sendChat("usage: !rizz <player>") return end
    ...
    stopOrbit() stopSpin() stopDance() stopLead(false) stopBam(false) stopAnnoy(false) stopFling(false) stopSwordKill(false)
    S.rizzTarget=t S.rizzActive=true S.mode="Rizz" S.rizzUsed={}
    ...
    S.rizzThread=task.spawn(function()
        while S.rizzActive and ROLE=="BOT" do
            task.wait(5)
            if not S.rizzActive then break end
            if not t.Parent or not t.Character then
                sendChat("target left, rizz over")
                stopRizz(false)
                break
            end
            local available={}
            for _,line in ipairs(RIZZ_LINES) do
                if not S.rizzUsed[line] then
                    table.insert(available,line)
                end
            end
            if #available==0 then
                S.rizzUsed={}
                for _,line in ipairs(RIZZ_LINES) do table.insert(available,line) end
            end
            local chosen=available[math.random(1,#available)]
            S.rizzUsed[chosen]=true
            sendChat(t.Name..", "..chosen)
        end
    end)
end
...
-- ANNOY LOOP (fixed: walks to front instead of teleporting)
task.spawn(function()
    while true do
        task.wait(0.05)
        if not S.annoyActive or ROLE~="BOT" or antiBan.detected then continue end
        if S.hidden then continue end
        local tg=S.annoyTarget
        if not tg or not tg.Parent then task.wait(0.4) continue end
        local th=getPlayerHRP(tg)
        local m=hrp()
        if not th or not m then task.wait(0.3) continue end
        if isInVoid(th.Position) or isInVoid(m.Position) then task.wait(0.3) continue end
        local lk=th.CFrame.LookVector
        local fl=Vector3.new(lk.X,0,lk.Z)
        if fl.Magnitude>0.01 then fl=fl.Unit else fl=Vector3.new(0,0,-1) end
        local fr=th.Position+fl*AFD
        local ds=(m.Position-fr).Magnitude
        local h=hum()
        if not h then continue end
        if rotationOwner~="FaceTarget" and S.annoyActive then setRotationOwner("FaceTarget") end
        -- Only teleport if target got WAY ahead (teleported/respawned). Otherwise walk.
        if ds>120 then
            if os.clock()-S.trollTpCD>3 then
                S.trollTpCD=os.clock()
                pcall(function()
                    m.CFrame=CFrame.new(fr+Vector3.new(0,1,0))
...
SK_CFG={CHASE_DIST=16,JUMP_DIST=12,BACKPEDAL_DIST=8,FLANK_DIST=5,SWING_CD=0.75,STRAFE_CD=0.55,REACTION_MIN=0.08,REACTION_MAX=0.18,PREDICT_TIME=0.18,HITBOX_DODGE_DIST=8,DMG_WINDOW=1.2,DMG_THRESHOLD=6,KILL_RETURN_DELAY=0.7,JUMP_INTERVAL=1.2,MIRROR_JUMP_WINDOW=0.35,FLANK_SIDE_BIAS=-1,CIRCLE_BIAS=0.75,FIGHT_TICK=0.07,MOVE_AHEAD=9}
...
function skSwing()local now=os.clock()if now-S.swordKillLastSwing<SK_CFG.SWING_CD*(0.9+math.random()*0.3)then return end S.swordKillLastSwing=now local tool=skEquipSword()if tool then pcall(function()tool:Activate()end)end end
...
if tHRP and tHRP.Position.Y>m.Position.Y+4.5 then shouldJump=true end
end
...
function skMoveTo(pos)
if not pos then return end
local h=hum()
if not h then return end
pcall(function()h:MoveTo(pos)end)
end
function skTick()
...
sendChat(pick({"got em","gg","target down","hes done","easy","next"}))
task.delay(SK_CFG.KILL_RETURN_DELAY,function()skUnequipAfterKill()stopSwordKill(false)teleportToHost()end)
end
return
end
...
if dmg>=SK_CFG.DMG_THRESHOLD then
S.swordKillRetreating=true
S.swordKillRetreatUntil=now+1.0
end
end
...
if enemySword then
local sd=(enemySword.Position-myPos).Magnitude
if sd<SK_CFG.HITBOX_DODGE_DIST then swordClose=true end
end
...
local back=-toTarget
local side=playerLeft*SK_CFG.FLANK_SIDE_BIAS
moveDir=(back*0.85+side*0.4).Unit
elseif swordClose and dist<11 then
moveDir=(playerLeft*0.9+toTarget*0.2).Unit
elseif dist>SK_CFG.CHASE_DIST then
local targetOffset=tPos+playerLeft*4
local chaseDir=targetOffset-myPos
chaseDir=Vector3.new(chaseDir.X,0,chaseDir.Z)
if chaseDir.Magnitude>0.1 then moveDir=chaseDir.Unit else moveDir=toTarget end
elseif dist>SK_CFG.JUMP_DIST then
moveDir=(playerLeft*0.5+toTarget*0.5).Unit
elseif dist>SK_CFG.BACKPEDAL_DIST then
moveDir=(playerLeft*SK_CFG.CIRCLE_BIAS+toTarget*0.2).Unit
elseif dist>SK_CFG.FLANK_DIST then
if not onUnguardedSide then
moveDir=(playerLeft*0.9+toTarget*0.1).Unit
else
moveDir=toTarget
end
elseif behind then
moveDir=toTarget
elseif not onUnguardedSide then
moveDir=(playerLeft*0.95+toTarget*0.05).Unit
else
local side=playerLeft*0.4
moveDir=(toTarget*0.7+side).Unit
end
if moveDir then
local dest=myPos+moveDir*SK_CFG.MOVE_AHEAD
if hasFloorBelow(dest) then skMoveTo(dest) else skMoveTo(tPos) end
end
if not S.swordKillRetreating and dist<SK_CFG.CHASE_DIST then skSwing() end
if grounded then skDoJump() end
end
...
S.swordKillThread=task.spawn(function()while S.swordKillActive and ROLE=="BOT"do local ok,err=pcall(skTick)if not ok then warn("[swordkill]",err)end task.wait(SK_CFG.FIGHT_TICK)end end)
...
function startLend(name, sc)
    ...
    if ROLE ~= "BOT" then
        if botLogRef then pushLog(botLogRef, "host cmd only works in BOT role", C.red) end
        return
    end
    if sc > 500 then
        sc = 500
        sendChat("max is 500s, using 500")
    end
    local t = getPlayer(name)
    if not t then sendChat(pick(R_.notfound)) return end
    if t == pl then sendChat(pick(R_.self)) return end
    if hostFilter.userId and t.UserId == hostFilter.userId then
        sendChat("already host rn, updating timer")
    end
    if not S.lending then
        if hostFilter.userId then
            originalHost.name = hostFilter.name
            originalHost.userId = hostFilter.userId
        elseif S.hostName then
            originalHost.name = S.hostName
            local hp = getPlayer(S.hostName)
            if hp then originalHost.userId = hp.UserId end
        end
    end
    S.lending = true
    S.hostName = t.Name:lower()
    hostFilter.name = t.Name:lower()
    hostFilter.userId = t.UserId
    S.lendEnd = os.clock() + sc
    sendChat(pick({"listening to ","host is now ","transferring to "})..t.Name.." for "..math.floor(sc).."s")
    ...
    S.lendThread = task.spawn(function()
        ...
        while S.lending and ROLE == "BOT" do
            task.wait(0.5)
            local left = S.lendEnd - os.clock()
            if left <= 0 then
                restoreHost()
                break
            elseif left <= 5 and not warned then
                warned = true
                sendChat("5s left for "..t.Name)
            end
        end
    end)
end
...
elseif field=="distance"or field=="dist"then if d>=0 then sendChat(p.Name.." is "..d.." studs away")else sendChat("no position for "..p.Name)end
elseif field=="hp"or field=="health"then sendChat(p.Name.." hp: "..ph.."/"..pmax)
elseif field=="friend"then sendChat(p.Name.." friend of you: "..tostring(isFriend))
elseif field=="display"or field=="name"then sendChat(p.Name.." display: "..p.DisplayName)
elseif field then sendChat(pick(R_.infoerr))
else task.spawn(function()sendChat(p.Name.." (@"..p.DisplayName..")")
task.wait(0.7)sendChat("id: "..p.UserId)
task.wait(0.7)sendChat("joined "..fmtDate(joined).." ("..fmtAge(accAge).." ago)")
task.wait(0.7)if d>=0 then sendChat("distance: "..d.." studs")end
task.wait(0.7)sendChat("hp: "..ph.."/"..pmax)
task.wait(0.7)local extras={}if isFriend then extras[#extras+1]="friend of you"end if isHost then extras[#extras+1]="current host"end if #extras>0 then sendChat("note: "..table.concat(extras,", "))end end)end end end
elseif cmd=="orbit"then startOrbit(args~=""and args or nil)elseif cmd=="unorbit"then stopOrbit()sendChat(pick(R_.unorbit))elseif cmd=="sit"then stopSpin()stopDance()stopLead(false)stopBam(false)stopAnnoy(false)stopFling(false)stopSwordKill(false)stopRizz(false)S.sitting=true S.waypoints=nil S.cachedPath=nil S.lastMovePos=nil S.committedTarget=nil local h=hum()if h then pcall(function()h.WalkSpeed=0 h.JumpPower=0 h.Sit=true h:ChangeState(Enum.HumanoidStateType.Seated)end)end sendChat(pick(R_.sit))elseif cmd=="stand"then S.sitting=false local h=hum()if h then pcall(function()h.Sit=false h:ChangeState(Enum.HumanoidStateType.GettingUp)h.WalkSpeed=BS h.JumpPower=BJ end)end sendChat(pick(R_.stand))elseif cmd=="jump"then local h=hum()if h then h.Jump=true sendChat(pick(R_.jump))end elseif cmd=="hide"then doHide(false)elseif cmd=="spawn"then doSpawn()elseif cmd=="dance"then playDance(tonumber(args)or 1)elseif cmd=="undance"then stopDance()sendChat(pick(R_.undance))elseif cmd=="spin"then startSpin(args)elseif cmd=="unspin"then stopSpin()sendChat(pick(R_.unspin))elseif cmd=="lead"then startLead(args)elseif cmd=="unlead"then stopLead(true)elseif cmd=="bam"then startBam(args)elseif cmd=="unbam"then stopBam(false)teleportToHost()sendChat(pick(R_.unbam))elseif cmd=="annoy"then startAnnoy(args)elseif cmd=="unannoy"then stopAnnoy(false)teleportToHost()sendChat(pick(R_.unannoy))elseif cmd=="fling"then startFling(args)elseif cmd=="unfling"then stopFling(false,false)teleportToHost()sendChat(pick(R_.unfling))elseif cmd=="swordkill"then startSwordKill(args)elseif cmd=="unswordkill"then stopSwordKill(true)elseif cmd=="antifling"then S.antiFling=true sendChat("antifling on")elseif cmd=="unantifling"then S.antiFling=false local c=pl.Character if c then for _,p in ipairs(c:GetDescendants())do if p:IsA("BasePart")then p.CanCollide=true end end end sendChat("antifling off")elseif cmd=="antitoolkill"then S.antiToolKill=true sendChat("antitoolkill on")elseif cmd=="unantitoolkill"then S.antiToolKill=false sendChat("antitoolkill off")elseif cmd=="rizz"then startRizz(args)elseif cmd=="unrizz"then stopRizz(true)elseif cmd=="math"then handleMath(args)elseif cmd=="whoishost"then local hh=getHost()if S.lending then sendChat("host is "..(hh and hh.Name or "?").." (temp, "..math.max(0,math.floor(S.lendEnd-os.clock())).."s left)")else sendChat("host is "..(hh and hh.Name or "none"))end
elseif cmd=="cmds"then sendSeq({"here ya go","!ask <msg> - talk to XcH","!ask <do something> - XcH will do it","!ask whats the name of this game","!gameinfo - what game is this","!gameinfo full | tips | controls | wiki","!similar - games like this one","!info <player> [id/age/joined/distance/hp/friend]","!stop | !recall | !follow","!backoff <n> | !getcloser <n>","!orbit <1-1000> | !unorbit","!lead <player> | !unlead","!sit | !stand | !jump","!hide | !spawn","!bam <player> | !unbam","!annoy <player> | !unannoy","!fling <player> | !unfling","!swordkill <player> | !unswordkill","!antifling | !unantifling","!antitoolkill | !unantitoolkill","!rizz <player> | !unrizz","!dance 1-4 | !undance","!spin 1-100 | !unspin","!math <num><op><num>","!say <text>","!host <player> <seconds, max 500> (creator only)","!stophost (creator only)","!whoishost","!cmds"},0.9)else sendChat(pick({"unknown cmd bro","dont know that one","try !cmds"}))end end
function processMessage(uid,text)if not text or text==""then return end 
    if uid~=pl.UserId and text:sub(1,1)~=commandPrefix then
        local speaker=P:GetPlayerByUserId(uid)
        local speakerName=speaker and speaker.Name or "Unknown"
        table.insert(S.chatLog, speakerName..": "..text)
        if #S.chatLog > 15 then table.remove(S.chatLog, 1) end
    end
    if ROLE~="BOT"and ROLE~="HOST"then return end 
    if ROLE=="BOT"and uid~=pl.UserId and not isFromHost(uid)then handleStrangerMsg(uid,text)return end 
    if not isFromHost(uid)then return end 
    local lowText=text:lower()if lowText:sub(1,4)=="!ask"then local prompt=trim(text:sub(5))if prompt==""then if ROLE=="BOT"then sendChat("usage: !ask <message>")end return end if isDuplicateMessage(uid,text)then return end task.spawn(function()handleAIChat(prompt)end)return end if ROLE~="BOT"then return end if text:sub(1,#commandPrefix)~=commandPrefix then return end if isDuplicateMessage(uid,text)then return end local body=trim(text:sub(#commandPrefix+1))if body==""then return end local sp=body:find("%s")local cmd,args if sp then cmd=body:sub(1,sp-1):lower()args=trim(body:sub(sp+1))else cmd=body:lower()args=""end
    if cmd=="host"or cmd=="stophost"then
        if not isCreatorUser(uid)then
            task.spawn(function()sendChat(pick({"only my boss can do that","nah not for you","creator only","youre not the boss","denied"}))end)
            return
        end
        if cmd=="host"then
            local sp2=args:find("%s")
            if not sp2 then task.spawn(function()sendChat("usage: !host <player> <seconds>")end) return end
            local pname=trim(args:sub(1,sp2-1))
            local secs=tonumber(trim(args:sub(sp2+1)))
            if not secs or secs<=0 then task.spawn(function()sendChat("need a valid seconds number")end) return end
            task.spawn(function()startLend(pname,secs)end)
        else
            task.spawn(function()stopLendNow()end)
        end
        return
    end
    task.spawn(function()local ok,err=pcall(handleCommand,cmd,args)if not ok then warn("[MyPanel] cmd error:",err)if botLogRef then pushLog(botLogRef,"err: "..tostring(err):sub(1,60),C.red)end end end)end
...
apiTab=makeTab("Api Keys",3,function()
if apiUnlocked then
slideToPage(apiPage,apiTab)
else
pwOv.Visible=true
