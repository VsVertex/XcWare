...
- "make urself immortal so u don't die" → [[CMD]]immortality[[/CMD]] on it
- "become mortal again" → [[CMD]]mortal[[/CMD]] ok
...
AI_CMD_WHITELIST={spin=true,dance=true,hide=true,spawn=true,jump=true,sit=true,stand=true,orbit=true,lead=true,bam=true,annoy=true,swordkill=true,fling=true,backoff=true,getcloser=true,recall=true,follow=true,stop=true,info=true,unspin=true,unorbit=true,unbam=true,unannoy=true,unfling=true,unswordkill=true,unlead=true,undance=true,antifling=true,unantifling=true,antitoolkill=true,unantitoolkill=true,immortality=true,mortal=true}
...
if S.mode=="SwordKill"or S.mode=="Bam"or S.mode=="Annoy"or S.mode=="Fling"then return end
...
task.spawn(function()while true do task.wait(0.05)if S.immortal and ROLE=="BOT"then local c=pl.Character if c then local h=c:FindFirstChildOfClass("Humanoid")if h then pcall(function()h:SetStateEnabled(Enum.HumanoidStateType.Dead, false) h.Health=0 h.MaxHealth=0 end)end end end end end)
...
if myHum.Health<=0 and not S.immortal then return end
...
local CMDS={{"── AI ──",true},{"!ask <msg> - talk to XcH",false},{"!ask <do something> - XcH will do it",false},{"!gameinfo - what game is this",false},{"!gameinfo full | tips | controls | wiki",false},{"!similar - games like this one",false},{"!info <player> [field]",false},{"!stop | !recall | !follow",false},{"!backoff <n> | !getcloser <n>",false},{"!orbit <1-1000> | !unorbit",false},{"!lead <player> | !unlead",false},{"!sit | !stand | !jump",false},{"!hide | !spawn",false},{"!bam <player> | !unbam",false},{"!annoy <player> | !unannoy",false},{"!fling <player> | !unfling",false},{"!swordkill <player> | !unswordkill",false},{"!antifling | !unantifling",false},{"!antitoolkill | !unantitoolkill",false},{"!immortality | !mortal",false},{"!dance 1-4 | !undance",false},{"!spin 1-100 | !unspin",false},{"!math <num><op><num>",false},{"!say <text>",false},{"!cmds",false}}
