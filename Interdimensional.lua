...
local TOOL_NAME = "My Panel"
local GUI_NAME = "InterDimensionalPanelGUI"
local STORAGE_NAME = "MyPanelStorage"
local BASE_SPEED = 16
local BASE_JUMP = 50
local PH_OFFSET = 8 * 3600
local FOLLOW_DIST = 8
local EMERGENCY_DIST = 70
local ORBIT_R = 14
local AFK_T = 20
local VOID_Y = -300
local VOID_RANGE = 4000
local REPATH_INTERVAL = 0.40
local DEDUP_WINDOW = 1.0
local LOOK_RANGE = 8
local STUCK_NOCLIP_TIME = 0.7
local LEAD_ARRIVE_DIST = 5
local LEAD_HOST_MAX_DIST = 45
local AI_TICK = 0.03
local PLAYER_DODGE_RANGE = 7
local WAYPOINT_REACH = 2.5
local VOID_THRESHOLD = -75
local OBSTACLE_LOOKAHEAD = 7
local STEEP_EDGE_DROP = 14
local SLIM_BLOCK_MIN = 1.5
local MICRO_CAM_MIN_WAIT = 3.0
local MICRO_CAM_MAX_WAIT = 7.0
local MICRO_CAM_ANGLE_MIN = 8
local MICRO_CAM_ANGLE_MAX = 22
local MIRROR_JUMP_MIN_DELAY = 0.15
local MIRROR_JUMP_MAX_DELAY = 0.42
local MIRROR_JUMP_COOLDOWN = 0.55
local PATH_MAX_ATTEMPTS = 3
local PATH_FAIL_COOLDOWN = 0.35
local BAM_DIST_BEHIND = 5.0
local BAM_OFFSET = 3.0
local BAM_CYCLE = 0.22
local BAM_TP_RANGE = 14
local ANNOY_FRONT_DIST = 3.5
local ANNOY_TP_RANGE = 22
local ANNOY_MSG_MIN_WAIT = 3.5
local ANNOY_MSG_MAX_WAIT = 7.5
-- FLING
local FLING_SPIN_X = 1500    -- deg/sec on X axis
local FLING_SPIN_Y = 2200    -- deg/sec on Y axis
local FLING_SPIN_Z = 1800    -- deg/sec on Z axis
local FLING_DETECT_VEL = 120 -- studs/s = flung
local FLING_DETECT_DIST = 30 -- studs moved in one frame = flung
local FLING_MAX_DURATION = 8 -- max seconds before timeout
local FLING_DETECT_WARMUP = 0.3 -- seconds before detection kicks in
...
-- ===== FLING (gyro spin inside target body) =====
...
startFling = function(name)
    ...
    -- Put bot into physics state so it doesn't try to walk/collide normally
    local h = hum()
...
task.spawn(function()
    while true do
        ...
        -- Gyro spin on all 3 axes at insane speed
        local t = os.clock()
        ...
        -- Also nudge all limbs into the target's volume by rotating the rig
        -- (already handled since HRP CFrame drags the whole rig)
        -- DETECT SUCCESS
