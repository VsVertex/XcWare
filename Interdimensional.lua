...
function runProtectTick()
    ...
    if S.hidden or S.sitting then return end
    if S.mode=="SwordKill" or S.mode=="Bam" or S.mode=="Annoy" or S.mode=="Fling" or S.mode=="Rizz" or S.mode=="Lead" or S.mode=="Orbit" then return end
    local now=os.clock()
    local hh=getHostHRP()
    ...
    for _,p in ipairs(P:GetPlayers())do
        ...
        if toward.Magnitude<0.1 then continue end
        toward=toward.Unit
        local speedAlong=toward:Dot(flatV)
        if not p:GetAttribute("_pxProtectDir")then
            p:SetAttribute("_pxProtectDir",math.sign(speedAlong))
            p:SetAttribute("_pxProtectFlips",0)
            p:SetAttribute("_pxProtectLastFlip",0)
        else
            local lastDir=p:GetAttribute("_pxProtectDir")
            local curDir=math.sign(speedAlong)
            if curDir~=0 and lastDir~=0 and curDir~=lastDir then
                local lastFlip=p:GetAttribute("_pxProtectLastFlip") or 0
                if now-lastFlip>0.25 then
                    p:SetAttribute("_pxProtectFlips",(p:GetAttribute("_pxProtectFlips") or 0)+1)
                    p:SetAttribute("_pxProtectLastFlip",now)
                end
                p:SetAttribute("_pxProtectDir",curDir)
            end
        end
        local flips=p:GetAttribute("_pxProtectFlips") or 0
        if flips>=3 then
            p:SetAttribute("_pxProtectFlips",0)
            S.protectTarget=p
...
elseif cmd=="gender"then
    ...
    if not sp2 then sendChat("usage: !gender <player> <m/f/they>")return end
    local pname=trim(args:sub(1,sp2-1))
    local g=trim(args:sub(sp2+1)):lower()
    if g~="m" and g~="f" and g~="they" then sendChat("gender must be m, f, or they")return end
    local p=getPlayer(pname)
    if not p then sendChat(pick(R_.notfound))return end
    sendChat("noted for "..p.Name)
elseif cmd=="genderclear"then
