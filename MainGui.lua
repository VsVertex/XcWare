...
-- NEW: RAINBOW RAPID FLOATING MATRIX OVERLAY
local MatrixOverlay = Instance.new("Frame", MainFrame)
MatrixOverlay.Size = UDim2.new(1, 100, 1, 100)
...
MatrixOverlay.BackgroundTransparency = 1 
MatrixOverlay.ZIndex = 5 
MatrixOverlay.ClipsDescendants = false 
...
PassBox.FocusLost:Connect(function(enter)
    if enter then
        if PassBox.Text == "XcWareIsBadass" then
            ...
            if _G.StartCredits then _G.StartCredits() end
        else
            PassBox.Text = ""
...
_G.HandleCommand = function(msg)
    ...
    elseif command == "-unspin" then
        if LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart:FindFirstChild("XcWare_Spin") then
            LocalPlayer.Character.HumanoidRootPart:FindFirstChild("XcWare_Spin"):Destroy()
...
SidebarProfiles.Size = UDim2.new(1, 0, 0, 120)
SidebarProfiles.Position = UDim2.new(0, 0, 0, 0)
...
PP_Scroll.ScrollingDirection = Enum.ScrollingDirection.X 
...
if LocalPlayer.Name == "Crixcrix000" then
    ...
    ChatToDiscordInput.FocusLost:Connect(function(enter)
        if enter and ChatToDiscordInput.Text ~= "" then
            ...
            local data = {["embeds"] = {{["title"] = "💬 Dev Message Sent",["description"] = msg,["color"] = 65280,["footer"] = {["text"] = "XcWare Dev Panel | User: " .. LocalPlayer.Name},["timestamp"] = os.date("!%Y-%m-%dT%H:%M:%SZ")}}}
            pcall(function()
                HttpService:PostAsync(devWebhook, HttpService:JSONEncode(data))
