...
-- ADDED: Customized Backpack System (Bottom Center)
...
-- [[ BACKPACK SYSTEM START ]]
local BackpackFrame = Instance.new("Frame")
local BackpackOpen = Instance.new("TextButton")
local BackpackGrid = Instance.new("UIGridLayout")
local BackpackHide = Instance.new("TextButton")

BackpackOpen.Name = "BackpackOpen"
BackpackOpen.Parent = ScreenGui
BackpackOpen.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
BackpackOpen.Position = UDim2.new(0.5, -25, 1, -65)
BackpackOpen.Size = UDim2.new(0, 50, 0, 50)
BackpackOpen.Font = Enum.Font.Code
BackpackOpen.Text = "+"
BackpackOpen.TextColor3 = Color3.fromRGB(255, 255, 255)
BackpackOpen.TextSize = 30
Instance.new("UICorner", BackpackOpen).CornerRadius = UDim.new(1, 0)
local OpenStroke = Instance.new("UIStroke", BackpackOpen)
OpenStroke.Color = Color3.fromRGB(0, 255, 70)
OpenStroke.Thickness = 2

BackpackFrame.Name = "BackpackFrame"
BackpackFrame.Parent = ScreenGui
BackpackFrame.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
BackpackFrame.Position = UDim2.new(0.5, -200, 1, 20) -- Starts hidden offscreen
BackpackFrame.Size = UDim2.new(0, 400, 0, 180)
BackpackFrame.ClipsDescendants = true
Instance.new("UICorner", BackpackFrame).CornerRadius = UDim.new(0, 12)
local BPStroke = Instance.new("UIStroke", BackpackFrame)
BPStroke.Color = Color3.fromRGB(255, 255, 255)
BPStroke.Transparency = 0.5

BackpackHide.Name = "BackpackHide"
BackpackHide.Parent = BackpackFrame
BackpackHide.Size = UDim2.new(0, 25, 0, 25)
BackpackHide.Position = UDim2.new(1, -30, 0, 5)
BackpackHide.BackgroundTransparency = 1
BackpackHide.Text = "X"
BackpackHide.TextColor3 = Color3.fromRGB(255, 0, 80)
BackpackHide.Font = Enum.Font.Code
BackpackHide.TextSize = 18

local SlotContainer = Instance.new("Frame", BackpackFrame)
SlotContainer.Size = UDim2.new(1, -20, 1, -40)
SlotContainer.Position = UDim2.new(0, 10, 0, 35)
SlotContainer.BackgroundTransparency = 1

BackpackGrid.Parent = SlotContainer
BackpackGrid.CellPadding = UDim2.new(0, 8, 0, 8)
BackpackGrid.CellSize = UDim2.new(0, 68, 0, 60)
BackpackGrid.HorizontalAlignment = Enum.HorizontalAlignment.Center

for i = 1, 10 do
    local slot = Instance.new("Frame", SlotContainer)
    slot.Name = "Slot" .. i
    ...
    num.TextYAlignment = Enum.TextYAlignment.Bottom
    
    -- Glitch animation for numbers
    task.spawn(function()
        while task.wait(math.random(2, 5)) do
            local originalPos = num.Position
            num.Position = originalPos + UDim2.new(0, math.random(-2,2), 0, math.random(-2,2))
            num.TextColor3 = GetGlitchColor()
            task.wait(0.1)
            num.Position = originalPos
            num.TextColor3 = Color3.fromRGB(100, 100, 100)
        end
    end)
end

local bpVisible = false
local function ToggleBP()
    bpVisible = not bpVisible
    local targetPos = bpVisible and UDim2.new(0.5, -200, 1, -260) or UDim2.new(0.5, -200, 1, 20)
    TweenService:Create(BackpackFrame, iOS_Out, {Position = targetPos}):Play()
    PlayClick()
end

BackpackOpen.Activated:Connect(ToggleBP)
BackpackHide.Activated:Connect(ToggleBP)
-- [[ BACKPACK SYSTEM END ]]
