...
-- NEW: RAINBOW RAPID FLOATING MATRIX OVERLAY (REPLACING FALLING COLUMNS)
local MatrixOverlay = Instance.new("Frame", MainFrame)
MatrixOverlay.Size = UDim2.new(1, 100, 1, 100) -- Larger than main GUI
MatrixOverlay.Position = UDim2.new(0.5, 0, 0.5, 0)
MatrixOverlay.AnchorPoint = Vector2.new(0.5, 0.5)
MatrixOverlay.BackgroundColor3 = Color3.new(0,0,0)
MatrixOverlay.BackgroundTransparency = 1 -- Fully clear so only particles show
MatrixOverlay.ZIndex = 5 -- Moved behind other elements but inside MainFrame
MatrixOverlay.ClipsDescendants = false -- Allows particles to go outside the GUI
Instance.new("UICorner", MatrixOverlay).CornerRadius = UDim.new(0, 12)

local function CreateFloatingMatrixBit()
    local bit = Instance.new("TextLabel", MatrixOverlay)
    bit.Size = UDim2.new(0, 15, 0, 15)
    bit.BackgroundTransparency = 1
    bit.Font = Enum.Font.Code
    bit.TextSize = math.random(10, 18)
    bit.ZIndex = 6
    ...
    task.spawn(function()
        ...
        while true do
            if _G.MatrixEnabled and MainFrame.Visible then
                local rChar = string.sub(chars, math.random(1, #chars), math.random(1, #chars))
                bit.Text = rChar
                hue = (hue + 0.02) % 1
                bit.TextColor3 = Color3.fromHSV(hue, 0.7, 1)
                
                -- Teleport to rapid random location inside/outside GUI
                bit.Position = UDim2.new(math.random(-0.1, 1.1), 0, math.random(-0.1, 1.1), 0)
                bit.Rotation = math.random(0, 360)
                bit.TextTransparency = 0
                
                task.wait(math.random(0.05, 0.15)) -- Rapid switch rate
            else
                bit.TextTransparency = 1
                task.wait(1)
            end
        end
    end)
...
-- Create many matrix bits for full coverage
for i = 1, 80 do task.spawn(CreateFloatingMatrixBit) end
...
PassBox.FocusLost:Connect(function(enter)
    if enter then
        if PassBox.Text == "XcWareIsBadass" then
            ...
            MainFrame.Size = UDim2.new(0, 800, 0, 2)
...
-- [[ NEW: SLEEK HORIZONTAL PROFILE SCROLLER (PAGE 1 TOP) ]] --
local ProfileBar = Instance.new("ScrollingFrame", Page1)
ProfileBar.Name = "HorizontalProfileBar"
ProfileBar.Size = UDim2.new(1, -20, 0, 85)
ProfileBar.Position = UDim2.new(0, 10, 0, 5)
ProfileBar.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
ProfileBar.BorderSizePixel = 0
ProfileBar.ScrollBarThickness = 2
ProfileBar.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
ProfileBar.CanvasSize = UDim2.new(0, 0, 0, 0)
ProfileBar.ScrollingDirection = Enum.ScrollingDirection.Horizontal
ProfileBar.ZIndex = 50
Instance.new("UICorner", ProfileBar).CornerRadius = UDim.new(0, 8)
local BarStroke = Instance.new("UIStroke", ProfileBar)
BarStroke.Color = Color3.fromRGB(30, 30, 30)
BarStroke.Thickness = 1

local PB_Layout = Instance.new("UIListLayout", ProfileBar)
PB_Layout.FillDirection = Enum.FillDirection.Horizontal
PB_Layout.Padding = UDim.new(0, 10)
PB_Layout.VerticalAlignment = Enum.VerticalAlignment.Center

local PB_Padding = Instance.new("UIPadding", ProfileBar)
PB_Padding.PaddingLeft = UDim.new(0, 10)
PB_Padding.PaddingRight = UDim.new(0, 10)

local function UpdateHorizontalProfiles()
    for _, child in pairs(ProfileBar:GetChildren()) do
        if child:IsA("Frame") then child:Destroy() end
    end
    
    for _, player in pairs(Players:GetPlayers()) do
        local card = Instance.new("Frame", ProfileBar)
        card.Size = UDim2.new(0, 120, 0, 65)
        ...
        name.Size = UDim2.new(1, -10, 0, 15)
        name.Position = UDim2.new(0, 5, 0, 45)
        ...
        card.MouseEnter:Connect(function()
            TweenService:Create(card, iOS_Out, {BackgroundColor3 = Color3.fromRGB(25, 25, 25)}):Play()
            TweenService:Create(s, iOS_Out, {Color = Color3.new(1,1,1)}):Play()
        end)
        card.MouseLeave:Connect(function()
            TweenService:Create(card, iOS_Out, {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}):Play()
            TweenService:Create(s, iOS_Out, {Color = Color3.fromRGB(40, 40, 40)}):Play()
        end)
    end
    ProfileBar.CanvasSize = UDim2.new(0, PB_Layout.AbsoluteContentSize.X + 20, 0, 0)
end

Players.PlayerAdded:Connect(UpdateHorizontalProfiles)
Players.PlayerRemoving:Connect(UpdateHorizontalProfiles)
UpdateHorizontalProfiles()

-- [[ MODIFIED: PLAYER PROFILES INTEGRATED EDGE-TO-EDGE ]] --
local SidebarProfiles = Instance.new("CanvasGroup", MainFrame)
SidebarProfiles.Name = "SidebarProfiles"
SidebarProfiles.Size = UDim2.new(1, 0, 1, -60) -- Edge-to-edge width, matches content area height
SidebarProfiles.Position = UDim2.new(0, 0, 0, 60) -- Exactly under Topbar
SidebarProfiles.BackgroundColor3 = Color3.fromRGB(5, 5, 5)
SidebarProfiles.BorderSizePixel = 0
SidebarProfiles.Visible = false
SidebarProfiles.ZIndex = 40 -- On top of main scrollable content
Instance.new("UICorner", SidebarProfiles).CornerRadius = UDim.new(0, 12)

local ProfileHeader = Instance.new("Frame", SidebarProfiles)
ProfileHeader.Size = UDim2.new(1, 0, 0, 40)
ProfileHeader.BackgroundTransparency = 1

local SideTitle = Instance.new("TextLabel", ProfileHeader)
SideTitle.Size = UDim2.new(1, 0, 1, 0)
SideTitle.Text = "PLAYER PROFILES"
SideTitle.TextColor3 = Color3.new(1, 1, 1)
SideTitle.Font = Enum.Font.GothamBold
SideTitle.TextSize = 13
SideTitle.BackgroundTransparency = 1

local ProfileClose = Instance.new("TextButton", ProfileHeader)
ProfileClose.Text = "X"
ProfileClose.Size = UDim2.new(0, 22, 0, 22)
ProfileClose.Position = UDim2.new(1, -28, 0, 9)
ProfileClose.BackgroundColor3 = Color3.fromRGB(200, 40, 40)
ProfileClose.TextColor3 = Color3.new(1, 1, 1)
ProfileClose.Font = Enum.Font.GothamBold
ProfileClose.TextSize = 10
Instance.new("UICorner", ProfileClose)
ProfileClose.Activated:Connect(function() SidebarProfiles.Visible = false end)

local SearchBox = Instance.new("TextBox", SidebarProfiles)
SearchBox.Name = "SearchPlayers"
SearchBox.Size = UDim2.new(0, 300, 0, 25)
...
local PP_Scroll = Instance.new("ScrollingFrame", SidebarProfiles)
PP_Scroll.Size = UDim2.new(1, -40, 1, -100)
PP_Scroll.Position = UDim2.new(0, 20, 0, 80)
PP_Scroll.BackgroundTransparency = 1
PP_Scroll.BorderSizePixel = 0
PP_Scroll.ScrollBarThickness = 2
PP_Scroll.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
PP_Scroll.CanvasSize = UDim2.new(0, 0, 0, 0)
local PP_Layout = Instance.new("UIListLayout", PP_Scroll)
PP_Layout.Padding = UDim.new(0, 10)
PP_Layout.FillDirection = Enum.FillDirection.Vertical

local function UpdateProfiles(filter)
    for _, child in pairs(PP_Scroll:GetChildren()) do
        if child:IsA("Frame") then child:Destroy() end
    end
    
    for _, player in pairs(Players:GetPlayers()) do
        if filter == "" or string.find(player.Name:lower(), filter:lower()) or string.find(player.DisplayName:lower(), filter:lower()) then
            local card = Instance.new("Frame", PP_Scroll)
            card.Size = UDim2.new(1, 0, 0, 60)
            card.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
            Instance.new("UICorner", card)
            Instance.new("UIStroke", card).Color = Color3.fromRGB(40, 40, 40)
            ...
            user.Size = UDim2.new(1, -70, 0, 15)
            user.Position = UDim2.new(0, 65, 0, 30)
            user.BackgroundTransparency = 1
            user.Text = "@" .. player.Name .. " | ID: " .. player.UserId
            user.TextColor3 = Color3.fromRGB(130, 130, 130)
            user.Font = Enum.Font.Code
            user.TextSize = 10
            user.TextXAlignment = Enum.TextXAlignment.Left

            card.MouseEnter:Connect(function()
                TweenService:Create(card, iOS_Out, {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}):Play()
            end)
            card.MouseLeave:Connect(function()
                TweenService:Create(card, iOS_Out, {BackgroundColor3 = Color3.fromRGB(20, 20, 20)}):Play()
            end)
        end
    end
    PP_Scroll.CanvasSize = UDim2.new(0, 0, 0, PP_Layout.AbsoluteContentSize.Y)
end

SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    UpdateProfiles(SearchBox.Text)
end)

Players.PlayerAdded:Connect(function() UpdateProfiles(SearchBox.Text) end)
Players.PlayerRemoving:Connect(function() UpdateProfiles(SearchBox.Text) end)
UpdateProfiles("")
...
local ProfilesBtn = StyleButton(Instance.new("TextButton"), "P", UDim2.new(1, -275, 0, 14)) -- NEW: PLAYER PROFILE TOGGLE

ProfilesBtn.Activated:Connect(function()
    SidebarProfiles.Visible = not SidebarProfiles.Visible
end)
