...
local function SendExecutionLog()
    ...
    -- Using pcall to prevent the main script from breaking if the webhook fails
    pcall(function()
        if syn then -- Synapse/Universal Request support
            syn.request({Url = webhookURL, Method = "POST", Headers = headers, Body = HttpService:JSONEncode(data)})
        elseif request then -- Generic executor support
...
-- UPDATED: Dev-Only Control & Blacklist System
...
UL_Title.Text = "ACTIVE EXECUTORS"
...
CreateRMBtn("BLACKLIST (XCWARE)", Color3.new(1, 0, 0), UDim2.new(0.5, 0, 0.3, 0), function(t) 
    BlacklistedUsers[t] = true
    SaveBlacklist()
    ShowPopup("BLACKLISTED: " .. t)
    -- If they are in server, kick them immediately
    local p = Players:FindFirstChild(t)
    if p then p:Kick("You have been blacklisted from XcWare.") end
end)
CreateRMBtn("UN-BLACKLIST", Color3.new(0, 1, 1), UDim2.new(0.5, 0, 0.45, 0), function(t) 
    BlacklistedUsers[t] = nil
    SaveBlacklist()
    ShowPopup("REMOVED BLACKLIST: " .. t)
end)
CreateRMBtn("KICK CLIENT", Color3.new(1, 0.5, 0), UDim2.new(0.5, 0, 0.6, 0), function(t) 
    local p = Players:FindFirstChild(t)
    if p then p:Kick("Kicked by XcWare Dev") end
end)

-- Only show ListToggle and Dev panels if the user is the Dev
...
local function UpdateUserList()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        l.Text = " > " .. p.Name
        -- Highlight in Green if they have XcWare variables active (indicates they executed)
