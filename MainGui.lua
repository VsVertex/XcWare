...
-- NEW: STATIC EFFECT & TV OPENING ANIMATION LOGIC
local StaticOverlay = Instance.new("Frame", MainFrame)
StaticOverlay.Size = UDim2.new(1, 0, 1, 0)
StaticOverlay.BackgroundColor3 = Color3.new(0,0,0)
StaticOverlay.BackgroundTransparency = 0.8
StaticOverlay.ZIndex = 40
StaticOverlay.ClipsDescendants = true
Instance.new("UICorner", StaticOverlay).CornerRadius = UDim.new(0, 12)

local StaticLabel = Instance.new("TextLabel", StaticOverlay)
StaticLabel.Size = UDim2.new(1.2, 0, 1.2, 0)
StaticLabel.Position = UDim2.new(-0.1, 0, -0.1, 0)
StaticLabel.BackgroundTransparency = 1
StaticLabel.TextColor3 = Color3.new(1,1,1)
StaticLabel.TextTransparency = 0.7
StaticLabel.Font = Enum.Font.Code
StaticLabel.TextSize = 8
StaticLabel.TextWrapped = true

local Scanline = Instance.new("Frame", StaticOverlay)
...
task.spawn(function()
    ...
    while true do
        ...
        Scanline.Position = UDim2.new(0, 0, 0, math.random(0, 500))
        task.wait(0.05)
    end
end)
...
PassBox.FocusLost:Connect(function(enter)
    if enter then
        if PassBox.Text == "XcWareIsBadass" then
            ...
            -- OLD TV OPENING ANIMATION
            MainFrame.Visible = true
            MainFrame.GroupTransparency = 1
            MainFrame.Size = UDim2.new(0, 800, 0, 2) -- Start as a thin line
            StaticOverlay.BackgroundTransparency = 0 -- Full static
            
            task.wait(0.1)
            MainFrame.GroupTransparency = 0
            
            -- Stage 1: Horizontal flicker expand
            TweenService:Create(MainFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Size = UDim2.new(0, 800, 0, 500)
            }):Play()
            
            -- Stage 2: Fade out static to reveal GUI
            TweenService:Create(StaticOverlay, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
...
-- PAGE 1 CONTENT REMOVED (BLANK AS REQUESTED)
-- EditorScrolling, CodeEditor, StorageScroll, and Action Buttons are not created for Page 1.
