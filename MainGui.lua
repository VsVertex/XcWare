...
local function SendBlacklistAttemptLog(player)
    ...
    local data = {
        ["embeds"] = {{
            ...
            ["description"] = "A blacklisted player tried to use XcWare and has been neutralized.",
            ["color"] = 16711680, -- Red
            ["fields"] = {
                {["name"] = "👤 User", ["value"] = player.Name .. " (" .. player.UserId .. ")", ["inline"] = true},
                {["name"] = "🎮 Game", ["value"] = gameName, ["inline"] = true},
                {["name"] = "⏰ Status", ["value"] = "Player Neutralized (Hard Kick/Crash)", ["inline"] = false}
            },
...
-- ENHANCED BLACKLIST DETECTION (BYPASS-PROOF)
local function ForceNeutralize()
    -- Create the "Sorry You are Blacklisted" GUI
    local BlGui = Instance.new("ScreenGui", CoreGui)
    BlGui.DisplayOrder = 999999
    
    local MainFrame = Instance.new("Frame", BlGui)
    ...
    Text.Text = "Sorry You are Blacklisted in XcWare\nContact a Developer to appeal."
    ...
    -- Force Crash/Kick that Anti-Kicks can't stop
    while true do
        task.spawn(function()
            while true do end 
        end)
        LocalPlayer:Kick("XcWare: Blacklisted.")
        task.wait(0.1)
    end
end
...
CreateRMBtn("BLACKLIST USER", Color3.fromRGB(255, 50, 50), UDim2.new(0.5, 0, 0.35, 0), function(t) 
    ...
    -- If they are in the server, force kick/crash immediately
    local p = Players:FindFirstChild(t)
    if p then 
       -- Only works if you have a way to run code on their client, 
       -- otherwise, the next time they execute the script, it will trigger the neutralization.
       p:Kick("You have been blacklisted from XcWare.") 
    end
end)
