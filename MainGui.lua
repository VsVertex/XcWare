...
-- NEW: RAINBOW RAPID MATRIX OVERLAY (REPLACING STATIC)
...
MatrixOverlay.BackgroundTransparency = 0.6
MatrixOverlay.ZIndex = 40
MatrixOverlay.ClipsDescendants = true
Instance.new("UICorner", MatrixOverlay).CornerRadius = UDim.new(0, 12)

local function CreateMatrixColumn()
    local column = Instance.new("TextLabel", MatrixOverlay)
    column.Size = UDim2.new(0, 12, 1, 0)
    ...
    task.spawn(function()
        ...
        while true do
            ...
            column.Text = str
            hue = (hue + 0.05) % 1
            column.TextColor3 = Color3.fromHSV(hue, 0.8, 1)
            column.Position = UDim2.new(math.random(), 0, -1, 0)
            local fall = TweenService:Create(column, TweenInfo.new(math.random(1.5, 3), Enum.EasingStyle.Linear), {Position = UDim2.new(column.Position.X.Scale, 0, 1, 0)})
            fall:Play()
            fall.Completed:Wait()
        end
    end)
...
for i = 1, 25 do task.spawn(CreateMatrixColumn) end
...
PassBox.FocusLost:Connect(function(enter)
    if enter then
        if PassBox.Text == "XcWareIsBadass" then
            ...
            MatrixOverlay.BackgroundTransparency = 0 
            ...
            TweenService:Create(MatrixOverlay, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
